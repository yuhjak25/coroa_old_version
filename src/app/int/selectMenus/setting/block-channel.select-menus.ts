...
const blockChannelSelect: SelectMenuData = {
  customId: "blockChannelSelect",
  run: async (client, int) => {
    const guild = int.guild;
    const app = client.user;
    const blockChannel = int.values;
    try {
      if (blockChannelSelect) {
        const selectBlockChannel = await SettingModel.findOneAndUpdate(
          {
            guild_id: guild?.id,
          },
          { "settings.channel_block": blockChannel },
          { new: true }
        );
      }
      ...
      const channels = channelsCollection?.map((channel) => ({
        ...
        value: channel.id,
        default: blockChannel.includes(channel.id),
      }));
...
export default blockChannelSelect;
